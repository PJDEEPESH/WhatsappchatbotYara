...
def analyze_user_intent(user_text):
    """Analyze user intent with improved accuracy"""
    today_str = date.today().strftime("%Y-%m-%d")
    weekday_str = date.today().strftime("%A")
    
    system_prompt = f"""Current Date: {today_str} ({weekday_str}).

You are analyzing user queries for a Buenos Aires recommendation bot.

CRITICAL RULES:
1. If user asks about FOOD/DRINKS (cafe, coffee, falafel, burger, pizza, etc.) → category: "restaurant" or "cafe"
2. If user asks about SHOPPING/ITEMS (laptop, phone, clothes, etc.) → category: "shop"
3. If user asks about MUSIC/DANCE (salsa, techno, jazz, reggaeton, etc.) → category: "event", specific_keywords: ["salsa" or "techno" etc.]
4. If user asks about BARS/NIGHTLIFE → category: "bar"
5. If user asks about DATES/ROMANTIC → target_mood: "romantic", category: "restaurant"
6. If user JUST says "hi"/"hello" with NO request → is_greeting: true

Return JSON with:
{{
  "is_greeting": boolean,
  "date_range": {{"start": "YYYY-MM-DD", "end": "YYYY-MM-DD"}} or null,
  "target_mood": "romantic" | "chill" | "party" | null,
  "category": "event" | "bar" | "restaurant" | "cafe" | "shop" | "museum" | null,
  "specific_keywords": ["salsa", "techno", "falafel", etc.] or []
}}
...
"""
    ...
    try:
        ...
        logger.info(f"AI Intent: {result}")
        return result
    except Exception as e:
...
def generate_just_for_you(user_age, item_name, item_desc, item_mood):
    ...
    try:
        prompt = f"Write a 1-sentence recommendation for a {user_age} year old. Venue: {item_name}. Vibe: {item_mood}. Start with '✨ Just for you:'."
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7,
        ...
        return f"✨ Just for you: Perfect for a {user_age} year old looking for a {item_mood} vibe!"

def generate_closing_message(user_query):
    """Generate closing message"""
    try:
        prompt = f"User asked: '{user_query}'. I sent recommendations. Write a SHORT closing asking if satisfied. Use emoji. Max 15 words."
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": "You are Yara."}, 
                      {"role": "user", "content": prompt}],
...
def build_search_query_smart(table, ai_data):
    ...
    if keywords:
        for kw in keywords:
            kw_wild = f"%{kw}%"
            if table == 'events':
                conditions.append("(title ILIKE %s OR description ILIKE %s OR music_type ILIKE %s)")
                args.extend([kw_wild, kw_wild, kw_wild])
            else:
    ...
    if category:
        ...
        if table == 'events':
            ...
            conditions.append("(name ILIKE %s OR description ILIKE %s OR description ILIKE %s)")
        args.extend([cat_wild, cat_wild, cat_wild] if table != 'events' else [cat_wild, cat_wild])
...
def smart_search(conn, table, ai_data):
    """Execute smart search with fallback"""
    try:
        # Try with all filters
        query, args = build_search_query_smart(table, ai_data)
        ...
        with conn.cursor() as cur:
            ...
            # Fallback: Try without mood/category, keep date/keywords
            if ai_data.get('target_mood') or ai_data.get('category'):
                fallback_ai = {
                    'date_range': ai_data.get('date_range'),
                    'specific_keywords': ai_data.get('specific_keywords', [])
                }
                query, args = build_search_query_smart(table, fallback_ai)
                cur.execute(query, tuple(args))
                results = cur.fetchall()
                
                if results:
                    logger.info(f"Found {len(results)} results with fallback")
                    return results
...
def ask_chatgpt_fallback(user_input, ai_data):
    ...
    category = ai_data.get('category', '')
    mood = ai_data.get('target_mood', '')
    keywords = ', '.join(ai_data.get('specific_keywords', []))
    date_str = ai_data.get('date_range', {}).get('start', '')
    
    context = f"""You are Yara, an expert Buenos Aires tourist guide.
...
- Date: {date_str if date_str else 'not specified'}

SITUATION: Our database doesn't have specific matches for this request.

YOUR TASK: Provide 2-3 SHORT, SPECIFIC recommendations for Buenos Aires that match what they're looking for. Be HELPFUL and ACCURATE. Keep response under 100 words."""
    ...
    try:
        ...
        return "I don't have specific matches in my database right now, but I'd love to help! Can you tell me more about what you're looking for? 😊"

def send_followup_message(user_id):
    try:
        logger.info(f"⏰ Follow-up to {user_id}")
        send_whatsapp_message(user_id, "Hey! Still here if you need more recommendations! 👋")
...
def process_message_thread(sender, text):
    ...
    try:
        ...
        # Handle greeting (if they already gave name/age)
        if ai_data.get('is_greeting') and user_name:
        ...
        if ai_data.get('date_range'):
            search_events = True
        elif ai_data.get('category') in ['event', 'party', 'show', 'concert']:
            search_events = True
        elif ai_data.get('specific_keywords'):
            # Check if keywords are music-related (events) or food/place-related (businesses)
            music_keywords = ['salsa', 'techno', 'jazz', 'reggaeton', 'house', 'rock', 'concert']
            keywords = ai_data.get('specific_keywords', [])
            if any(k.lower() in music_keywords for k in keywords):
                search_events = True
            else:
                search_businesses = True
        elif ai_data.get('category') in ['bar', 'restaurant', 'cafe', 'shop', 'museum']:
            search_businesses = True
        else:
        ...
        if search_events:
            ...
            if events:
                ...
                all_results.append({
                    'type': 'events',
                    'items': events,
                    'intro': f"Here is what's happening around {ai_data.get('date_range', {}).get('start')}:" 
                             if ai_data.get('date_range') else "Here are some events for you:"
                })
        ...
        # ✅ CRITICAL: Build complete message queue BEFORE sending
        if found_something:
            # Collect all items with context
            all_items_with_context = []
            for result_group in all_results:
                for i, item in enumerate(result_group['items']):
                    all_items_with_context.append({
                        'group_type': result_group['type'],
                        'group_intro': result_group['intro'],
                        'item': item,
                        'is_first_in_group': (i == 0)
                    })
            
            # Submit ALL AI calls in parallel
            jfy_futures = []
            for item_context in all_items_with_context:
                item = item_context['item']
                if item_context['group_type'] == 'events':
                    future = executor.submit(
                        generate_just_for_you,
                        user_age,
                        item['title'],
                        item['description'],
                        item.get('mood', 'social')
                    )
                else:
                    future = executor.submit(
                        generate_just_for_you,
                ...
                jfy_futures.append((future, item_context))
            
            # Build message queue
            messages_to_send = []
            for future, item_context in jfy_futures:
                # Add intro if first item
                if item_context['is_first_in_group']:
                    messages_to_send.append((item_context['group_intro'], None))
                
                # Get AI result
                just_for_you = future.result()
                item = item_context['item']
                
                # Build message
                if item_context['group_type'] == 'events':
                    display_date = item.get('event_date') if item.get('event_date') else f"Every {item.get('recurring_day')}"
                    caption = (
                        f"*{item.get('title')}*\n\n"
                        f"📍 Location: {item.get('location')}\n"
                        f"🕒 Time: {item.get('event_time')}\n"
                        f"📅 Date: {display_date}\n"
                        f"🎵 Music: {item.get('music_type')}\n"
                        f"📝 {item.get('description')}\n"
                        f"📸 {item.get('instagram_link')}\n\n"
                        f"{just_for_you}"
                    )
                    messages_to_send.append((caption, item.get('image_url')))
                else:
                    msg = (
                        f"*{item.get('name')}*\n"
                        f"📍 {item.get('location')}\n\n"
                    ...
                    messages_to_send.append((msg, None))
            
            # Add closing LAST (GUARANTEED!)
            closing = generate_closing_message(text)
            messages_to_send.append((closing, None))
            
            # Send all messages sequentially
            for i, (body, media_url) in enumerate(messages_to_send):
                if i > 0:
                    time.sleep(0.5)
                send_whatsapp_message(sender, body, media_url=media_url)
        else:
            # Fallback
            fallback_text = ask_chatgpt_fallback(text, ai_data)
            send_whatsapp_message(sender, fallback_text)
...
def health_check():
    return {"status": "healthy", "service": "yara-bot"}, 200

if __name__ == "__main__":
    logger.info("🚀 Yara Bot Starting...")
