...
# ==============================================================================
# 🧠 AI & UTILS (UNCHANGED - This part is already perfect)
# ==============================================================================
...
def analyze_user_intent(user_text):
    ...
    system_prompt = (
        ...
        "   - Examples:\n"
        "     * User says 'artistic events' → inferred_keywords: ['art', 'gallery', 'exhibition', 'museum', 'theatre', 'performance', 'cultural']\n"
        "     * User says 'a place for a date' → inferred_keywords: ['romantic', 'intimate', 'cozy']\n"
        "     * User says 'somewhere to go with friends' → inferred_keywords: ['social', 'group', 'casual', 'fun']\n"
        
        "EXAMPLES:\n"
        "User: 'cafe'\n"
        "→ {category: 'cafe', user_language: 'en'}\n"
        "User: 'artistic events this weekend'\n"
        "→ {category: 'event', date_range: {...}, user_language: 'en', inferred_keywords: ['art', 'gallery', 'exhibition', 'museum', 'theatre', 'performance', 'cultural']}\n"
    ...
    try:
        ...
        if not data.get('user_language'): data['user_language'] = 'en'
        logger.info(f"🧠 AI Analysis: {data}")
        return data
    except Exception as e:
        logger.error(f"AI Intent Error: {e}")
...
def translate_text(text, target_language):
    if target_language == 'en' or not text: return text
    try:
        lang_map = {'es': 'Spanish', 'pt': 'Portuguese', 'fr': 'French', 'de': 'German', 'it': 'Italian', 'ru': 'Russian', 'ar': 'Arabic', 'he': 'Hebrew', 'hi': 'Hindi', 'te': 'Telugu', 'ta': 'Tamil', 'ko': 'Korean', 'ja': 'Japanese', 'zh': 'Chinese'}
        lang_name = lang_map.get(target_language, 'English')
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": f"Translate the following text to {lang_name}. Maintain original tone. Only return the translation."}, {"role": "user", "content": text}],
...
def generate_just_for_you(user_age, item_name, item_desc, item_mood, social_context=None, user_language='en'):
    ...
    return "✨ Just for you: A great spot to check out!"
...
# ==============================================================================
# ⚙️ UPDATED & SMARTER SEARCH LOGIC
# ==============================================================================

def build_search_query(table, ai_data, strictness_level):
    """
    UPDATED: This function is now much smarter for both simple and complex queries.
    """
    query = f"SELECT * FROM public.{table} WHERE 1=1"
    args = []
    
    # 1. Build search terms from ALL available context
    search_terms = []
    
    # Direct keywords from user
    if ai_data.get('specific_keywords'):
        search_terms.extend(ai_data.get('specific_keywords'))
    # Inferred keywords from AI for abstract concepts (like 'artistic')
    if ai_data.get('inferred_keywords'):
        search_terms.extend(ai_data.get('inferred_keywords'))
    # Mood
    if ai_data.get('target_mood'):
    ...
    # Clean and deduplicate all collected terms
    search_terms = list(set([t.lower() for t in search_terms if t and len(t) > 2]))
    logger.info(f"🔍 Search Terms (Level {strictness_level}): {search_terms}")

    # Handle simple, single-word queries like "cafe" or "bar"
    if len(search_terms) == 1 and table == 'businesses':
        term = search_terms[0]
        query += " AND (type ILIKE %s OR name ILIKE %s OR description ILIKE %s)"
        args.extend([f"%{term}%"] * 3)
        logger.info(f"📊 Built SIMPLE SQL Query for '{term}'")
        query += " LIMIT 5"
        return query, args

    # --- DATE LOGIC (for events - UNCHANGED) ---
    date_range = ai_data.get('date_range') or {}
    if table == 'events' and date_range:
        start, end = date_range.get('start'), date_range.get('end')
        if start and end:
    ...
    query += " ORDER BY event_date ASC LIMIT 5" if table == 'events' else " LIMIT 5"
    logger.info(f"📊 Built COMPLEX SQL Query: {query[:200]}...")
...
def smart_search(conn, table, ai_data):
    ...
    with conn.cursor() as cur:
        cur.execute(query, tuple(args))
        results = cur.fetchall()
        if results: logger.info(f"✅ Found {len(results)} results (Loose)")
        else: logger.warning(f"⚠️ No results in {table}")
        return results if results else []

# (Twilio Utils and Fallback functions are UNCHANGED)
def send_whatsapp_message(to, body, media_url=None):
    if not TWILIO_WHATSAPP_NUMBER: return
    try:
        message_data = {'from_': TWILIO_WHATSAPP_NUMBER, 'to': to, 'body': body}
        if media_url: message_data['media_url'] = media_url
        twilio_client.messages.create(**message_data)
    except Exception as e:
        logger.error(f"❌ Twilio Error: {e}")
...
def ask_chatgpt_expert_fallback(user_input, ai_data, user_language='en'):
    # This function remains unchanged.
    return "I couldn't find specific matches, but I'm looking for more ideas for you!"


# ==============================================================================
# ⚙️ MAIN PROCESS - UPDATED WITH SMARTER SEARCH PRIORITY
# ==============================================================================
...
def process_message_thread(sender, text):
    ...
    try:
        ...
        # Greeting & Name/Age Capture logic is unchanged
        if ai_data.get('is_greeting') and step != 'ask_name_age':
            # ... (greeting logic)
            return
        if step == 'welcome' or step == 'ask_name_age':
            # ... (onboarding logic)
            # This part will re-run analyze_user_intent after getting name/age
            return

        found_something = False
        
        # --- THIS IS THE UPDATED SEARCH PRIORITY LOGIC ---

        # Determine user's primary interest
        is_asking_for_place = any(cat in ai_data.get('category', '') for cat in ['bar', 'restaurant', 'cafe', 'club', 'shop', 'museum']) or social_context
        is_asking_for_event = ai_data.get('date_range') or any(cat in ai_data.get('category', '') for cat in ['event', 'concert', 'show', 'party']) or 'artistic' in text.lower()

        # PRIORITY 1: BUSINESSES if explicitly asked or implied
        if is_asking_for_place and not is_asking_for_event:
            businesses = smart_search(conn, 'businesses', ai_data)
            if businesses:
                found_something = True
                # (Send business results - code is the same as before)
                intro = translate_text("Found these spots for you:", user_language)
                send_whatsapp_message(sender, intro)
                for b in businesses:
        ...
        # PRIORITY 2: EVENTS if explicitly asked or implied
        if is_asking_for_event:
            events = smart_search(conn, 'events', ai_data)
            if events:
                found_something = True
                # (Send event results - code is the same as before)
                intro = translate_text("Here are some events matching your vibe:", user_language)
                send_whatsapp_message(sender, intro)
                for e in events:
                    # ... formatting and sending logic ...
                    pass
        
        # FALLBACK: If the main priority search yielded nothing, try the other category
        if not found_something:
            if is_asking_for_place and not is_asking_for_event: # Tried businesses, now try events
                events = smart_search(conn, 'events', ai_data)
                if events: 
                    found_something = True
                    # (Send event results)
            elif is_asking_for_event and not is_asking_for_place: # Tried events, now try businesses
                businesses = smart_search(conn, 'businesses', ai_data)
                if businesses:
                    found_something = True
                    # (Send business results)
...
if __name__ == "__main__":
    ...
    print("✨ Enhanced Features:")
    print("   - Fully Multi-language (English DEFAULT)")
    print("   - Intelligent Abstract Search (e.g., 'artistic events', 'a place for a date')")
    print("   - Auto-translation of all content")
    print("   - Images with all recommendations")
    app.run(port=5000)